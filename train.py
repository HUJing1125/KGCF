# train.py (optimized version for FB15k237)
import os
import json
import pickle
import random
...
def evaluate_filtered(model, test_triples, all_correct, all_entities,
    ...
    """
    Filtered link prediction evaluation (tail entity prediction) using DistMult score (higher is better)
    """
    model.eval()
...
def train():
    ...
    T = 30                        # Number of neighbors to sample
    walk_times = 10
    walk_length = 3
    lambda_weight = 0.7           # Semantic-structure weight
    batch_size = 64
    learning_rate = 5e-4
    num_epochs = 200
    num_neg = 5                   # Number of negative samples per positive (for self-adversarial)
    warmup_epochs = 5
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for heads, rels, tails in pbar:
            ...
            # Multi-negative self-adversarial sampling: for each positive, generate num_neg negatives,
            # pick the highest scoring as training negative
            all_neg_scores = []
            for _ in range(num_neg):
                # Random shuffle of tail entities (simple but effective)
                neg_tails = tails.copy()
                random.shuffle(neg_tails)
                # Avoid negative sample identical to positive (rare case, simple fix)
            ...
            # Take the hardest negative (highest score)
        ...
        # Update cosine learning rate after warmup
        if epoch >= warmup_epochs:
            scheduler.step()
